...
import SpecTabsV5 from "./SpecTabsV5";
...
export default function ComparePageV5() {
  ...
  return (
    <main className="min-h-screen">
            ...
            <div className="flex-1">
              <div className="flex gap-[91px]">
                {products.map((product) => (
                  <div key={product.id} className="flex-1 flex items-center gap-[12px]">
                    <p
                      className="text-[14px] font-bold text-zinc-900 truncate"
                      style={{ fontFamily: "var(--font-sharp-sans)" }}
                    >
                      {product.title}
                    </p>
                    <button
                      className="shrink-0 rounded-full bg-zinc-900 px-4 py-1.5 text-[12px] font-bold text-white transition-colors hover:bg-zinc-800"
                      style={{ fontFamily: "var(--font-samsung-one)" }}
                    >
                      Buy Now
                    </button>
        ...
        <div className="flex gap-[91px]">
          {products.map((product) => (
            <div key={product.id} className="flex-1">
              <div className="h-[304px] lg:h-[320px] xl:h-[334px] min-[1600px]:h-[368px] rounded-[7px] bg-[#FAFAFA] flex items-center justify-center overflow-hidden">
                <Image
                  src={product.image}
                  alt={product.title}
                  width={240}
                  height={280}
                  className="object-contain h-auto w-auto max-h-[280px] lg:max-h-[296px] xl:max-h-[308px] min-[1600px]:max-h-[336px]"
                />
              </div>
      ...
      <div className="mt-[64px]">
        <SpecTabsV5 activeTab={activeTab} onTabChange={setActiveTab} isSticky={isSticky} showDifferences={showDifferences} setShowDifferences={setShowDifferences} />
      </div>
