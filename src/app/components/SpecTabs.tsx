...
export default function SpecTabs({ activeTab, onTabChange, isSticky }: SpecTabsProps) {
    ...
    <div className="max-w-[1440px] mx-auto px-[128px]">
      ...
      <div ref={specsRef} className="scroll-mt-[260px] flex flex-col gap-[32px]">
        {currentSpecs.map((spec, index) => (
          ...
          >
            <h3
              className="text-[20px] font-bold text-zinc-900 mb-[24px]"
              style={{ fontFamily: "var(--font-samsung-one)" }}
            >
              {spec.label}
            </h3>
            <div className="flex gap-[91px]">
              {spec.values.map((value, i) => (
                <div key={i} className="flex-1 relative">
                  {i > 0 && (
                    <div
                      className="absolute left-[-46px] top-1/2 -translate-y-1/2 w-px h-[24px] bg-[#DDDDDD]"
                      style={{
                        opacity: isVisible ? 1 : 0,
                        transition: `opacity 400ms cubic-bezier(0.16, 1, 0.3, 1) ${index * 60 + 150}ms`,
                      }}
                    />
                  )}
                  <div
                    className="h-[40px] flex items-center bg-[#FAFAFA] rounded-[4px] px-4"
                    style={{
                      opacity: isVisible ? 1 : 0,
                      transform: isVisible ? "scale(1)" : "scale(0.97)",
                      transition: `opacity 400ms cubic-bezier(0.16, 1, 0.3, 1) ${index * 60 + i * 40}ms, transform 400ms cubic-bezier(0.16, 1, 0.3, 1) ${index * 60 + i * 40}ms`,
                    }}
                  >
                    <p
